...
class ProgressiveManager extends Manager {
  ...
  hire(employee){
    ...
    else if(this.reports.length < 11 && this.reports.length > 3){
      this.title = "Mostly Manager";
    }
    else if(this.reports.length < 51 && this.reports.length > 10){
      this.title = "Manager";
    }
    else if(this.reports.length < 101 && this.reports.length > 50){
  ...
  //hire function with title increase 
  //fire function with $100 bonus
...
class Machine {
  ...
  makeWidgets(num){
    var numIndex = Math.floor(num / 50);
    
    this.widgets_made_count += num;
    this.wear_and_tear_count += numIndex;
  }
  fixMachine(){
